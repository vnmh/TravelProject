...
exports.read = function (req, res) {
   ...
   if (idAccount !== null && idAccount !== undefined && idAccount !== '') {
      ...
      Order.getOrderByIdWithIdAccount(req.query.idOrder, req.query.idAccount, function (err, order) {
         if (err) res.send(err);
...
exports.getLinkPayment = function (req, res) {
   ...
   Order.createOrder(newOrder, function (err, order) {
      ...
      let secureCode = MD5(
         `${process.env.MERCHANT_SITE_CODE} ${process.env.RETURN_URL} ${process.env.RECEIVER} ${
            process.env.TRANSACTION_INFO
         } ${newOrder.PIN} ${newOrder.totalPrice} vnd 1 0 0 0 0 Tour:${tour.titleTour
            .trim()
            .replace(/\s/g, '')}People-${newOrder.numberPeople}Children-${
            newOrder.numberChildren
         } ${newOrder.buyer.trim().replace(/\s/g, '')}*|*${newOrder.email
            .trim()
            .replace(/\s/g, '')}*|*${newOrder.phone.trim().replace(/\s/g, '')}*|*${address
            .trim()
            .replace(/\s/g, '')}  ${process.env.SECURE_PASS_SANDBOX}`
      );
      ...
      }&return_url=${process.env.RETURN_URL}&receiver=${process.env.RECEIVER}&transaction_info=${
         process.env.TRANSACTION_INFO
      }&order_code=${newOrder.PIN}&price=${
         newOrder.totalPrice
      }&currency=vnd&quantity=1&tax=0&discount=0&fee_cal=0&fee_shipping=0&order_description=Tour:${tour.titleTour
         .trim()
         .replace(/\s/g, '')}People-${newOrder.numberPeople}Children-${
         newOrder.numberChildren
      }&buyer_info=${newOrder.buyer.trim().replace(/\s/g, '')}*|*${newOrder.email
         .trim()
         .replace(/\s/g, '')}*|*${newOrder.phone.trim().replace(/\s/g, '')}*|*${address
         .trim()
