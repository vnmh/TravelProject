...
exports.verify = async (req, res, next) => {
   ...
   try {
      const verifyToken = await jwt.verify(req.query.ddSWuQzP8x2cHckmKxiK, 'ithoangtansecurity');
      const emailVerify = await jwt.verify(req.query.QZmWYU22y2zb2qZg8clJ, 'ithoangtansecurity');
   ...
   Accounts.getByEmailAndRole(email, 'user')
      .then(async (account) => {
         ...
         const html = `Hi there,
            ...
            verifyToken,
            'ithoangtansecurity'
         )}&QZmWYU22y2zb2qZg8clJ=${jwt.sign(account.email, 'ithoangtansecurity')}&wXvkihdDAD9D8FI9Nwpf=${jwt.sign(
            Date.now(),
            'ithoangtansecurity'
         )}">${process.env.FRONT_END}forgotPassword?${jwt.sign('verifyToken', 'ithoangtansecurity')}=${jwt.sign(
            verifyToken,
            'ithoangtansecurity'
         )}</a>
                        <br/><br/>
...
exports.forgotPasswordStep2 = (req, res, next) => {
   //change password
   try {
      const verifyToken = jwt.verify(req.query.ddSWuQzP8x2cHckmKxiK, 'ithoangtansecurity');
      const date = jwt.verify(req.query.wXvkihdDAD9D8FI9Nwpf, 'ithoangtansecurity');
      const email = jwt.verify(req.query.QZmWYU22y2zb2qZg8clJ, 'ithoangtansecurity');
