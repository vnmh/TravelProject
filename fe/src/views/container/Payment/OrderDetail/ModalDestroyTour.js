...
function DestroyTour(props) {
   ...
   console.log("maidev ~ file: ModalDestroyTour.js ~ line 30 ~ DestroyTour ~ destroyFee", destroyFee);

   useEffect(() => {
      // GET info price
      const dayOrderFromNow = Math.abs(moment().diff(props.orderDetail?.order?.departureDay, "days"));
      console.log("maidev ~ file: ModalDestroyTour.js ~ line 34 ~ useEffect ~ dayOrderFromNow", dayOrderFromNow);
      console.log(
         "maidev ~ file: ModalDestroyTour.js ~ line 49 ~ useEffect ~ moment().diff(props.orderDetail?.departureDay,)",
         moment().diff(props.orderDetail?.departureDay, "days")
      );
   ...
   const onFinish = (values) => {
      ...
      console.log(
         `ithoangtan -  ~ file: ModalDestroyTour.js ~ line 53 ~ onFinish ~ props.orderDetail`,
         props.orderDetail
      );
      ...
      props
         ...
         .then((res) => {
            ...
            if (props.user) history.push(USER_BOOKING);
   ...
   return (
      <Form form={form} onFinish={onFinish} className='d-flex flex-column'>
         <div style={{ maxHeight: "50vh", overflowY: "auto" }}>
            ...
            <p>
               {" "}
               Nếu TRAVEL TRIZEN không thực hiện được chuyến du lịch, du lịch Trizen Travel phải báo ngay cho khách hàng
               biết và thanh toán lại cho khách hàng toàn bộ số tiền khách hàng đã đóng trong vòng 3 ngày kể từ lúc việc
            ...
            <p>
               {" "}
               – Trường hợp hủy tour do sự cố khách quan như thiên tai, dịch bệnh, hoãn và hủy chuyến của các phương
               tiện vận chuyển công cộng… TRAVEL TRIZEN sẽ không chịu trách nhiệm bồi thường thêm bất kỳ chi phí nào
               ngoài việc hoàn trả tiền tour.
            </p>
            <p>
               – Trên đây là mức phạt hủy tối đa, chi phí này có thể được giảm tùy theo điều kiện của từng nhà cung cấp
               dịch vụ cho TRAVEL TRIZEN.
            </p>
            <p>
               {" "}
               – Thời gian hủy chuyến du lịch được tính cho ngày làm việc, không tính thứ 7, Chủ Nhật và các ngày Lễ,
               Tết.
            </p>
            ...
            <p>
               {" "}
               Khách hàng hủy Vé du lịch theo đúng những qui định trên, trong trường hợp khách thanh toán trực tuyến,
               nếu hủy Vé du lịch khách hàng sẽ chịu toàn bộ phí ngân hàng cho việc thanh toán tiền Vé du lịch.
            </p>
            <p>
               {" "}
               Việc hoàn trả tiền cho khách sẽ được TRAVEL TRIZEN thực hiện ngay sau khi ngân hàng thông báo tiền đã vào
               tài khoản của TRAVEL TRIZEN.
            </p>

            <p> Trường hợp bất khả kháng</p>
            <p>
               {" "}
               Nếu chương trình du lịch bị hủy bỏ hoặc thay đổi bởi một trong hai bên vì một lý do bất khả kháng (hỏa
               hoạn, thời tiết, tai nạn, thiên tai, chiến tranh, hoãn và hủy chuyến của các phương tiện vận chuyển công
               cộng…), thì hai bên sẽ không chịu bất kỳ nghĩa vụ bồi hoàn các tổn thất đã xảy ra và không chịu bất kỳ
               trách nhiệm pháp lý nào. Tuy nhiên mỗi bên có trách nhiệm cố gắng tối đa để giúp đỡ bên bị thiệt hại nhằm
               giảm thiểu các tổn thất gây ra vì lý do bất khả kháng.
            </p>

            <b>
               {" "}
               Note: Việc huỷ bỏ chuyến đi phải được thông báo trực tiếp với Công ty hoặc qua fax, email và phải được
               Công ty xác nhận. Việc huỷ bỏ bằng điện thoại không được chấp nhận.
            </b>
         </div>
         <Form.Item
         ...
         <Button loading={loading} type='primary' className='mt-2' size='large' danger htmlType='submit'>
            HỦY TOUR
         </Button>
      </Form>
   );
