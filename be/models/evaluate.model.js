...
Evaluate.getAllEvaluate = function (funcResult) {
   return new Promise(function (resolve, reject) {
      database
         .query('SELECT * FROM ' + databaseProduction + ".evaluates WHERE statusAction <> 'deleted';")
...
Evaluate.getEvaluateById = function (idEvaluate) {
   return new Promise(function (resolve, reject) {
      database
         .query('call ' + databaseProduction + `.spGetEvaluateById( '${idEvaluate}' ); `)
         .then((rows) => resolve(rows))
         .catch((err) => reject(err));
   });
};
...
Evaluate.getEvaluateByIdTour = function (idTour) {
   return new Promise(function (resolve, reject) {
      database
         .query('call ' + databaseProduction + `.spGetEvaluateByIdTour( '${idTour}' ); `)
         .then((rows) => resolve(rows))
         .catch((err) => reject(err));
   });
};
...
Evaluate.updateById = function (updateEvaluate) {
   ...
   return new Promise(function (resolve, reject) {
      database
         .query('UPDATE ' + databaseProduction + '.evaluates SET ? WHERE (idEvaluate = ?);', [
            updateEvaluate,
            updateEvaluate.idEvaluate
         ])
         .then((rows) => resolve(rows))
         .catch((err) => reject(err));
   });
...
Evaluate.remove = function (idEvaluate) {
   return new Promise(function (resolve, reject) {
      database
         .query('UPDATE ' + databaseProduction + ".evaluates SET `statusAction` = 'deleted' WHERE idEvaluate = ?", [
            idEvaluate
         ])
         .then((rows) => resolve(rows))
         .catch((err) => reject(err));
